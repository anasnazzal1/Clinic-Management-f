...
import { appointmentsApi, visitsApi, doctorsApi } from '@/lib/api';
...
import {
  ...
  Clock, FileText, TriangleAlert, Calendar, Search, X, ExternalLink,
} from 'lucide-react';
...
export const DoctorDashboard = () => {
  ...
  useEffect(() => {
    appointmentsApi.getAll().then(r => {
      ...
      data.forEach(a => {
        const p = a.patientId;
        if (p && !seen.has(p._id ?? p)) { seen.add(p._id ?? p); unique.push(p); }
      });
      setPatients(unique);
    }).catch(() => {});
  }, []);
  ...
  const goToPatient = (patientId: string) => navigate(`/doctor/patients/${patientId}`);
              ...
              <tbody>
                {pending.slice(0, 5).map(a => (
                  <tr key={a._id} className="border-b last:border-0">
                    <td className="py-2.5">
                      <button onClick={() => goToPatient(a.patientId?._id ?? a.patientId)} className="font-medium text-primary hover:underline">
                        {a.patientId?.name || '—'}
                      </button>
                    </td>
...
export const DoctorAppointmentsPage = () => {
  ...
  const handleSaveVisit = async () => {
    ...
    try {
      ...
      setAppts(prev => prev.map(a =>
        a._id === selectedAppt._id ? { ...a, status: 'completed', notes: visitForm.notes, diagnosis: visitForm.diagnosis } : a
      ));
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      setAppts(prev => prev.map(a => a._id === cancelTarget._id ? { ...a, status: 'cancelled' } : a));
      toast.success('Appointment cancelled.');
    } catch { toast.error('Failed to cancel appointment.'); }
    finally { setCancelTarget(null); }
  };

  const openFollowUp = (a: any) => { setFollowUpTarget(a); setFollowUpForm({ date: '', time: '' }); setFollowUpOpen(true); };
  ...
  const handleSaveFollowUp = async () => {
    ...
    const clash = appts.find(a =>
      ...
      a.date === followUpForm.date && a.time === followUpForm.time && a.status === 'pending'
    );
    if (clash) { toast.error('A pending appointment already exists for this patient at that slot.'); return; }
    ...
    } catch (e: any) { toast.error(e.response?.data?.message || 'Failed to book follow-up.'); }
  ...
  const ActionButtons = ({ a }: { a: any }) => {
    if (a.status === 'pending') return (
      <div className="flex items-center justify-end gap-1 flex-wrap">
        <Button size="sm" className="bg-success/15 text-success hover:bg-success/25 border-0 gap-1" onClick={() => openVisit(a)}>
          <CheckCircle2 className="w-3.5 h-3.5" /> Complete
        </Button>
        <Button size="sm" variant="ghost" className="text-destructive hover:text-destructive hover:bg-destructive/10 gap-1" onClick={() => setCancelTarget(a)}>
  ...
  const filtered = appts.filter(a => !search || (a.patientId?.name ?? '').toLowerCase().includes(search.toLowerCase()));
    ...
    <div className="space-y-6">
      ...
      <Card className="shadow-card">
        <CardContent className="pt-4 space-y-3">
          <div className="relative">
            <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
            <Input className="pl-9 pr-9" placeholder="Search patients by name..." value={search} onChange={e => setSearch(e.target.value)} />
            {search && (
              <button onClick={() => setSearch('')} className="absolute right-3 top-1/2 -translate-y-1/2 text-muted-foreground hover:text-foreground transition-colors" aria-label="Clear search">
                <X className="w-3.5 h-3.5" />
              </button>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {filtered.map(a => (
                  <tr key={a._id} className={`border-b last:border-0 transition-colors ${a.status === 'cancelled' ? 'opacity-50' : ''}`}>
                    <td className="py-2.5">
                      <button
                        onClick={() => navigate(`/doctor/patients/${a.patientId?._id || a.patientId}`)}
        ...
        <DialogContent>
          ...
          <div className="space-y-4">
            <div className="space-y-1">
              ...
              <Input value={visitForm.diagnosis} onChange={e => setVisitForm(f => ({ ...f, diagnosis: e.target.value }))} placeholder="e.g. Hypertension..." disabled={selectedAppt?.status !== 'pending'} />
            </div>
            <div className="space-y-1">
              <Label>Notes</Label>
              <Textarea rows={4} value={visitForm.notes} onChange={e => setVisitForm(f => ({ ...f, notes: e.target.value }))} placeholder="Clinical observations..." disabled={selectedAppt?.status !== 'pending'} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setVisitOpen(false)}>{selectedAppt?.status === 'pending' ? 'Cancel' : 'Close'}</Button>
            {selectedAppt?.status === 'pending' && (
              <Button onClick={handleSaveVisit} disabled={savingVisit} className="bg-success text-white hover:bg-success/90 border-0 gap-1">
        ...
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              Cancel the appointment for <span className="font-medium text-foreground">{cancelTarget?.patientId?.name}</span> on {cancelTarget?.date} at {cancelTarget?.time}? This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
      ...
      <Dialog open={followUpOpen} onOpenChange={setFollowUpOpen}>
        <DialogContent>
          ...
          {followUpTarget && (
            <div className="text-sm text-muted-foreground mb-2 bg-muted/40 rounded-lg px-3 py-2">
              Patient: <span className="font-medium text-foreground">{followUpTarget.patientId?.name}</span>
              {doctorInfo && <span className="ml-3 text-xs">Working hours: {doctorInfo.workingDays} · {doctorInfo.workingHours}</span>}
            </div>
          )}
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-1">
              <Label>Date</Label>
              <DatePicker value={followUpForm.date} onChange={v => setFollowUpForm(f => ({ ...f, date: v }))} disabled={d => d < new Date(new Date().setHours(0, 0, 0, 0))} />
            </div>
            <div className="space-y-1">
              <Label>Time</Label>
              <TimePicker value={followUpForm.time} onChange={v => setFollowUpForm(f => ({ ...f, time: v }))} />
            </div>
          </div>
          <DialogFooter>
            <Button variant="outline" onClick={() => setFollowUpOpen(false)}>Cancel</Button>
            <Button onClick={handleSaveFollowUp} disabled={savingFollowUp} className="gradient-primary border-0 text-primary-foreground gap-1">
