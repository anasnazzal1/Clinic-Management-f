...
import {
  ...
  Heart, Search, X, Calendar, Mail, ChevronDown, ChevronUp,
} from 'lucide-react';
...
const Landing = () => {
  ...
  const [expandedClinics, setExpandedClinics] = useState<Set<string>>(new Set(clinics.map(c => c.id)));
  ...
  const filteredClinics = useMemo(() => {
    ...
    return clinics.map(clinic => {
      const clinicDoctors = doctors.filter(d => {
        if (d.clinicId !== clinic.id) return false;
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="py-12 border-b">
        <div className="container mx-auto px-4 grid grid-cols-2 md:grid-cols-4 gap-6">
          {[
            { value: String(totalClinics), label: 'Departments' },
            { value: String(totalDoctors), label: 'Doctors' },
            { value: String(totalSpecs), label: 'Specializations' },
      ...
      <section id="doctors" className="py-10">
        <div className="container mx-auto px-4 space-y-8">
          {filteredClinics.length === 0 ? (
            ...
            filteredClinics.map(({ clinic, clinicDoctors }, ci) => (
              <motion.div
                key={clinic.id}
              ...
              >
                <Card className="shadow-card overflow-hidden">
                  ...
                  <button
                    ...
                    aria-expanded={expandedClinics.has(clinic.id)}
                  >
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className="flex flex-wrap gap-x-4 gap-y-1 mt-1.5 text-sm text-muted-foreground">
                            <span className="flex items-center gap-1.5"><Clock className="w-3.5 h-3.5 shrink-0" />{clinic.workingDays} · {clinic.workingHours}</span>
                          </div>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0 pt-1">
                        <Badge variant="secondary" className="text-xs">{clinicDoctors.length} doctor{clinicDoctors.length !== 1 ? 's' : ''}</Badge>
                      ...
                      >
                        <div className="border-t px-5 py-4">
                          {clinicDoctors.length === 0 ? (
                            <p className="text-sm text-muted-foreground py-4 text-center">No doctors match the current filter at this clinic.</p>
                          ) : (
            ...
            <>
              ...
              <div className="space-y-3 text-sm border-t pt-4">
                ...
                <div className="flex items-center gap-3 text-muted-foreground">
                  ...
                  <span>{clinics.find(c => c.id === selectedDoctor.clinicId)?.name} Department</span>
                </div>
                <div className="flex items-center gap-3 text-muted-foreground">
