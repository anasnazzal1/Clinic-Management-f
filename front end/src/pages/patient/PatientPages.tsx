import { appointments, doctors, clinics, visits } from '@/data/mockData';
...
export const PatientDashboard = () => {
  const { user } = useAuth();
  const patientId = user?.linkedId;
  const myAppts = appointments.filter(a => a.patientId === patientId);
  const upcoming = myAppts.filter(a => a.status === 'pending');
  const patient = { name: user?.name, email: user?.email };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid sm:grid-cols-3 gap-4">
        <Card className="shadow-card"><CardContent className="pt-5 flex items-center gap-4"><User className="w-8 h-8 text-primary" /><div><div className="font-medium text-foreground">{patient.name}</div><div className="text-xs text-muted-foreground">{patient.email}</div></div></CardContent></Card>
        ...
        <Card className="shadow-card"><CardContent className="pt-5 text-center"><div className="font-display text-3xl font-bold text-success">{myAppts.filter(a => a.status === 'completed').length}</div><div className="text-xs text-muted-foreground mt-1">Past Visits</div></CardContent></Card>
      </div>
    </div>
  );
};
            ...
            <tbody>
              {myAppts.map(a => (
                <tr key={a.id} className="border-b last:border-0">
                  <td className="py-2.5 font-medium text-foreground">{doctors.find(d => d.id === a.doctorId)?.name}</td>
              ...
              {myAppts.length === 0 && <tr><td colSpan={4} className="py-8 text-center text-muted-foreground">No appointments.</td></tr>}
            </tbody>
          </table>
        </CardContent>
      </Card>
...
export const PatientHistoryPage = () => {
  const { user } = useAuth();
  const patientId = user?.linkedId;
  const myVisits = visits.filter(v => v.patientId === patientId);
            ...
            <Card key={v.id} className="shadow-card">
              <CardContent className="pt-5">
                <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2 mb-3">
                  ...
                  <div className="text-sm text-muted-foreground">{doctors.find(d => d.id === v.doctorId)?.name}</div>
                </div>
